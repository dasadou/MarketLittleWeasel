...
// Function to create a regex based on conditions
function createRegex(conditions) {
  let regexString = conditions
      .map(cond => {
        if (cond.type === 'keyword') {
          return cond.value ? `(${cond.value.split(/\s+/).map(kw => kw.trim()).filter(kw => kw).join('|')})` : '';
        }
        return cond.value;
      })
      .join(' ');
...
// Function to hide articles that do not match the conditions
function hideArticles(startIndex) {
  const children = table.children;
  const regex = createRegex(conditions); // Create regex from conditions

  for (let i = startIndex; i < children.length; i++) {
    const childNode = children[i];
    extractArticleDetails(childNode); // Extract article details

    if (!regex.test(childNode.textContent)) { // If the article name does not match the regex
      childNode.style.display = "none"; // Hide the child node
    } else {
      childNode.style.display = ""; // Show the child node
    }
  }
}
...
function handleMessage(request, sender, sendResponse) {
  switch (request.message) {
    case 'start':
      conditions = request.conditions; // Store the conditions
